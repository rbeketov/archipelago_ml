import os
import io
import requests
from typing import Optional

from dotenv import load_dotenv
from pydub import AudioSegment

YA_SPEECH_TO_TEXT_URL = (
    "https://stt.api.cloud.yandex.net/speech/v1/stt:recognize?topic=general"
)


class SpeechToText:
    def __init__(self, api_key):
        self.api_key = api_key

        load_dotenv()
        os.environ["FFMPEG_PATH"] = "/opt/homebrew/bin/ffmpeg"

    def _convert_audio_mp3_to_opus(self, mp3_data) -> bytes:
        audio = AudioSegment.from_file(io.BytesIO(mp3_data), format="mp3")
        opus_data = audio.export(format="opus").read()
        return opus_data

    def get(self, audio_data: bytes) -> Optional[str]:
        opus_audio_data = self._convert_audio_mp3_to_opus(audio_data)

        headers = {
            "Authorization": self.api_key,
            "Content-Type": "audio/ogg",
        }

        response = requests.post(
            YA_SPEECH_TO_TEXT_URL, data=opus_audio_data, headers=headers
        )
        if response.status_code == 200:
            return response.json()["result"]
        return response.json()


if __name__ == "__main__":
    from app.config import Config

    config = Config()

    with open("audio_data/audio1424368168.mp3", "rb") as file:
        data = file.read()

    print(SpeechToText(config.env.API_KEY_SPEACH_KIT).get(data))
